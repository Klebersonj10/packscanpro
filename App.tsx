...
const FALLBACK_CITIES = [
  ...
  "SÃO GONÇALO / RJ", "NATAL / RN", "TERESINA / PI"
];
...
const App: React.FC = () => {
  ...
  const [editFormData, setEditFormData] = useState<(ExtractedData & { icComment?: string, isNewProspect?: boolean, isPotentialProject?: boolean }) | null>(null);
  const [zoomImage, setZoomImage] = useState<string | null>(null);
  const [notifications, setNotifications] = useState<AppNotification[]>([]);
  const [activePhotos, setActivePhotos] = useState<Record<string, number>>({});
  
  const [searchQuery, setSearchQuery] = useState('');
  const [biFilter, setBiFilter] = useState<'approved' | 'rejected' | 'pending' | 'new' | 'registered' | 'project' | null>(null);
    ...
    try {
      ...
      const remoteLists = (data || []).map(l => ({
        ...
        entries: (l.product_entries || []).map((e: any) => ({
          ...
          isNewProspect: e.is_new_prospect, 
          isPotentialProject: e.is_potential_project || false,
          checkedAt: new Date(e.created_at).toLocaleString('pt-BR'),
  ...
  const handleUpdateEntry = async (entryId: string) => {
    ...
    try {
      ...
      const updatePayload: any = {
        ...
        cnpj_raiz: raiz,
        is_new_prospect: editFormData.isNewProspect,
        is_potential_project: editFormData.isPotentialProject
      };
  ...
  const handleProcessImages = async (photos: string[]) => {
    ...
    try {
      ...
      const { data: dbExisting } = await supabase
        .from('product_entries').select('id').eq('cnpj_raiz', raiz).limit(1);
  ...
  const analytics = useMemo(() => {
    ...
    if (biFilter === 'approved' || biFilter === 'rejected' || biFilter === 'pending') {
      ...
      filteredEntriesByBI = all.filter(e => e.isNewProspect);
    } else if (biFilter === 'registered') {
      filteredEntriesByBI = all.filter(e => !e.isNewProspect);
    } else if (biFilter === 'project') {
      filteredEntriesByBI = all.filter(e => e.isPotentialProject);
    }
    ...
    const pdvRanking = Object.entries(pdvRaw).sort((a,b) => (b[1] as any) - (a[1] as any)).slice(0,5);
    const maxPdv = Math.max(...Object.values(pdvRaw) as number[], 1);

    const cityRaw = lists.reduce<any>((acc, l) => { acc[l.city] = (acc[l.city] || 0) + (l.entries?.length || 0); return acc; }, {});
    const cityRanking = Object.entries(cityRaw).sort((a,b) => (b[1] as any) - (a[1] as any)).slice(0,5);
    const maxCity = Math.max(...Object.values(cityRaw) as number[], 1);
    ...
    return {
      ...
      newCount: all.filter(e => e.isNewProspect).length,
      registeredCount: all.filter(e => !e.isNewProspect).length,
      projectCount: all.filter(e => e.isPotentialProject).length,
      pdvRanking,
      maxPdv,
      cityRanking,
      maxCity,
  ...
  return (
    <div className="min-h-screen bg-slate-50 pb-24 font-sans tracking-tight">
             ...
             <div className="grid gap-8">
                {currentList.entries.map(entry => (
                  <div key={entry.id} className="bg-white p-8 rounded-[40px] border border-slate-200 shadow-sm group hover:shadow-xl transition-all relative overflow-hidden">
                     ...
                     {entry.isPotentialProject && (
                        <div className="absolute top-0 right-[220px] px-6 py-2 rounded-b-2xl text-[10px] font-black uppercase italic tracking-widest shadow-sm bg-purple-600 text-white flex items-center gap-2">
                          <Sparkles className="w-3 h-3" /> Novo Projeto
                        </div>
                     )}
                        ...
                        <div className="flex-grow">
                           <div className="flex justify-between items-start mb-6 border-b border-slate-100 pb-4">
                                  ...
                                  <div className="space-y-3">
                                    ...
                                    <div className="flex gap-4 items-center mt-2">
                                       <label className="flex items-center gap-2 cursor-pointer">
                                          <input type="checkbox" className="w-3 h-3" checked={editFormData?.isNewProspect} onChange={e => setEditFormData(prev => prev ? {...prev, isNewProspect: e.target.checked} : null)} />
                                          <span className="text-[9px] font-black uppercase">Novo Prospect?</span>
                                       </label>
                                       <label className="flex items-center gap-2 cursor-pointer">
                                          <input type="checkbox" className="w-3 h-3" checked={editFormData?.isPotentialProject} onChange={e => setEditFormData(prev => prev ? {...prev, isPotentialProject: e.target.checked} : null)} />
                                          <span className="text-[9px] font-black uppercase text-purple-600">Possível Novo Projeto?</span>
                                       </label>
                                    </div>
                                  </div>
                                ) : (
                              ...
                              <div className="flex gap-2 shrink-0">
                                {editingEntryId === entry.id ? (
                                  ...
                                  <button onClick={() => { setEditingEntryId(entry.id); setEditFormData({...entry.data, icComment: entry.icComment, isNewProspect: entry.isNewProspect, isPotentialProject: entry.isPotentialProject}); }} className="bg-slate-50 text-slate-400 p-3 rounded-xl hover:text-blue-600 transition-colors"><Edit3 className="w-5 h-5" /></button>
                                )}
                           ...
                           <div className="mt-8 pt-6 border-t border-slate-100 flex flex-col md:flex-row items-center justify-between gap-4">
                             ...
                             <div className="flex gap-3 shrink-0">
                               ...
                               <button onClick={() => handleSetReviewStatus(entry.id, 'approved')} className={`p-4 rounded-2xl flex items-center gap-2 transition-all ${entry.reviewStatus === 'approved' ? 'bg-emerald-600 text-white shadow-lg' : 'bg-emerald-50 text-emerald-600 hover:bg-emerald-100'}`}>
          ...
          <div className="space-y-10 animate-in fade-in pb-16">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 py-6 border-b border-slate-100">
              <div className="flex items-center gap-5">
                ...
                <div>
                   <h2 className="text-3xl font-black uppercase italic tracking-tighter text-slate-900 leading-none">Relatório Master Comercial</h2>
            ...
            <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
              <button 
                ...
                className={`bg-white p-6 rounded-[35px] border flex flex-col items-center gap-2 transition-all active:scale-95 ${biFilter === 'approved' ? 'ring-4 ring-emerald-500/20 border-emerald-500 shadow-lg' : 'border-slate-50 shadow-sm'}`}
              >
                <ThumbsUp className={`w-8 h-8 ${biFilter === 'approved' ? 'text-emerald-500' : 'text-slate-300'}`} />
                <div className="text-center"><p className="text-[8px] font-black text-slate-300 uppercase italic">Aprovados</p><p className="text-2xl font-black text-emerald-600 leading-none">{analytics.approvedCount}</p></div>
              </button>

              <button 
                onClick={() => setBiFilter(biFilter === 'rejected' ? null : 'rejected')}
                className={`bg-white p-6 rounded-[35px] border flex flex-col items-center gap-2 transition-all active:scale-95 ${biFilter === 'rejected' ? 'ring-4 ring-rose-500/20 border-rose-500 shadow-lg' : 'border-slate-50 shadow-sm'}`}
              >
                <ThumbsDown className={`w-8 h-8 ${biFilter === 'rejected' ? 'text-rose-500' : 'text-slate-300'}`} />
                <div className="text-center"><p className="text-[8px] font-black text-slate-300 uppercase italic">Reprovados</p><p className="text-2xl font-black text-rose-500 leading-none">{analytics.rejectedCount}</p></div>
              </button>

              <button 
                onClick={() => setBiFilter(biFilter === 'pending' ? null : 'pending')}
                className={`bg-white p-6 rounded-[35px] border flex flex-col items-center gap-2 transition-all active:scale-95 ${biFilter === 'pending' ? 'ring-4 ring-amber-500/20 border-amber-500 shadow-lg' : 'border-slate-50 shadow-sm'}`}
              >
                <Clock className={`w-8 h-8 ${biFilter === 'pending' ? 'text-amber-500' : 'text-slate-300'}`} />
                <div className="text-center"><p className="text-[8px] font-black text-slate-300 uppercase italic">Pendentes</p><p className="text-2xl font-black text-amber-600 leading-none">{analytics.pendingCount}</p></div>
              </button>

              <button 
                onClick={() => setBiFilter(biFilter === 'new' ? null : 'new')}
                className={`bg-white p-6 rounded-[35px] border flex flex-col items-center gap-2 transition-all active:scale-95 ${biFilter === 'new' ? 'ring-4 ring-blue-500/20 border-blue-500 shadow-lg' : 'border-slate-50 shadow-sm'}`}
              >
                <Sparkles className={`w-8 h-8 ${biFilter === 'new' ? 'text-blue-500' : 'text-slate-300'}`} />
                <div className="text-center"><p className="text-[8px] font-black text-slate-300 uppercase italic">Novos</p><p className="text-2xl font-black text-blue-600 leading-none">{analytics.newCount}</p></div>
              </button>

              <button 
                onClick={() => setBiFilter(biFilter === 'registered' ? null : 'registered')}
                className={`bg-white p-6 rounded-[35px] border flex flex-col items-center gap-2 transition-all active:scale-95 ${biFilter === 'registered' ? 'ring-4 ring-slate-400/20 border-slate-400 shadow-lg' : 'border-slate-50 shadow-sm'}`}
              >
                <DatabaseZap className={`w-8 h-8 ${biFilter === 'registered' ? 'text-slate-500' : 'text-slate-300'}`} />
                <div className="text-center"><p className="text-[8px] font-black text-slate-300 uppercase italic">Na Base</p><p className="text-2xl font-black text-slate-700 leading-none">{analytics.registeredCount}</p></div>
              </button>

              <button 
                onClick={() => setBiFilter(biFilter === 'project' ? null : 'project')}
                className={`bg-white p-6 rounded-[35px] border flex flex-col items-center gap-2 transition-all active:scale-95 ${biFilter === 'project' ? 'ring-4 ring-purple-500/20 border-purple-500 shadow-lg' : 'border-slate-50 shadow-sm'}`}
              >
                <Sparkles className={`w-8 h-8 ${biFilter === 'project' ? 'text-purple-500' : 'text-slate-300'}`} />
                <div className="text-center"><p className="text-[8px] font-black text-slate-300 uppercase italic">Projetos</p><p className="text-2xl font-black text-purple-600 leading-none">{analytics.projectCount}</p></div>
              </button>
            </div>
            ...
            {biFilter && (
              <div className="bg-white p-10 rounded-[50px] border border-slate-200 shadow-xl space-y-8 animate-in slide-in-from-top-4 overflow-hidden">
                 <div className="flex flex-col md:flex-row items-center justify-between pb-4 border-b border-slate-50 gap-4">
                    <h3 className="text-sm font-black uppercase italic tracking-widest flex items-center gap-3">
                      <LayoutList className="w-5 h-5 text-blue-600" /> 
                      Categoria: <span className="text-blue-600">{biFilter.toUpperCase()}</span>
                    </h3>
                    <div className="flex flex-wrap justify-center gap-3">
                       {(biFilter === 'pending' || biFilter === 'new' || biFilter === 'registered') && currentUser?.role === 'admin' && (
                         <button 
                            onClick={async () => {
                               const pendentes = analytics.biFilteredEntries.filter(e => e.reviewStatus === 'pending');
                               if(pendentes.length === 0) {
                                  addNotification("Aviso", "Não há itens pendentes neste filtro.", "info");
                                  return;
                               }
                               if(window.confirm(`Deseja APROVAR todos os ${pendentes.length} itens pendentes deste filtro?`)) {
                                  setIsProcessing(true);
                                  try {
                                    await Promise.all(pendentes.map(e => handleSetReviewStatus(e.id, 'approved')));
                                    addNotification("Sucesso", `${pendentes.length} registros validados para o BI.`, "success");
                                  } catch (err) {
                                    addNotification("Erro", "Falha ao aprovar registros.", "warning");
                                  } finally {
                                    setIsProcessing(false);
                                  }
                               }
                            }}
                            className="bg-emerald-600 text-white px-5 py-3 rounded-2xl font-black text-[9px] uppercase shadow-lg shadow-emerald-100 flex items-center gap-2 hover:bg-emerald-700 transition-colors"
                         >
                            <CheckCircle className="w-4 h-4" /> Aprovar Pendentes {biFilter === 'new' ? 'Novos' : biFilter === 'registered' ? 'da Base' : ''}
                         </button>
                       )}
                         ...
                         <div className="flex items-center gap-5 min-w-0">
                            ...
                            <div className="min-w-0">
                               ...
                               <div className="flex gap-2 mt-1">
                                  <span className={`px-2 py-0.5 rounded-full text-[7px] font-black uppercase tracking-widest border ${e.isNewProspect ? 'bg-amber-50 text-amber-600 border-amber-100' : 'bg-blue-50 text-blue-600 border-blue-100'}`}>
                                    {e.isNewProspect ? 'Novo Prospect' : 'Já Cadastrado'}
                                  </span>
                                  <span className={`px-2 py-0.5 rounded-full text-[7px] font-black uppercase tracking-widest border ${e.reviewStatus === 'approved' ? 'bg-emerald-50 text-emerald-600 border-emerald-100' : e.reviewStatus === 'rejected' ? 'bg-rose-50 text-rose-500 border-rose-100' : 'bg-slate-100 text-slate-400 border-slate-200'}`}>
                                    {e.reviewStatus}
                                  </span>
                               </div>
                            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="bg-white p-10 rounded-[50px] border border-slate-50 shadow-sm space-y-8">
                <h3 className="text-[12px] font-black text-slate-400 uppercase tracking-widest flex items-center gap-4 italic leading-none">
                  <Building2 className="w-6 h-6 text-blue-600" /> Share de Fabricantes
                </h3>
              ...
              <div className="bg-white p-10 rounded-[50px] border border-slate-50 shadow-sm space-y-10">
                <h3 className="text-[12px] font-black text-slate-400 uppercase tracking-widest flex items-center gap-4 italic leading-none">
                  <Box className="w-6 h-6 text-purple-600" /> Moldagem Técnica
                </h3>
              ...
              <div className="bg-white p-10 rounded-[50px] border border-slate-50 shadow-sm space-y-8">
                <h3 className="text-[12px] font-black text-slate-400 uppercase tracking-widest flex items-center gap-4 italic leading-none">
                  <LayoutList className="w-6 h-6 text-emerald-600" /> Ranking por PDV
                </h3>
                <div className="space-y-6">
                    ...
                    <div key={i} className="space-y-3">
                      ...
                      <div className="h-2.5 bg-slate-100 rounded-full overflow-hidden">
                        <div 
                          ...
                          style={{ width: `${((count as number) / analytics.maxPdv) * 100}%` }} 
                        />
                      </div>
                    </div>
                  ))}
                  {analytics.pdvRanking.length === 0 && <p className="text-[10px] font-bold text-slate-300 uppercase italic">Nenhum dado disponível</p>}
                </div>
              </div>

              {/* Novo Ranking de Cobertura de Cidades */}
              <div className="bg-white p-10 rounded-[50px] border border-slate-50 shadow-sm space-y-8">
                <h3 className="text-[12px] font-black text-slate-400 uppercase tracking-widest flex items-center gap-4 italic leading-none">
                  <MapPin className="w-6 h-6 text-orange-600" /> Cobertura de Cidades
                </h3>
                <div className="space-y-6">
                    ...
                    <div key={i} className="space-y-3">
                      ...
                      <div className="h-2.5 bg-slate-100 rounded-full overflow-hidden">
                        <div 
                          ...
                          style={{ width: `${((count as number) / analytics.maxCity) * 100}%` }} 
                        />
                      </div>
                    </div>
                  ))}
                  {analytics.cityRanking.length === 0 && <p className="text-[10px] font-bold text-slate-300 uppercase italic">Nenhum dado disponível</p>}
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="space-y-6 animate-in slide-in-from-right-5 pb-10">
            <div className="flex flex-col md:flex-row justify-between items-center gap-4 pt-4">
              ...
              <div className="flex items-center gap-3 w-full md:w-auto">
                ...
                }} className="bg-emerald-600 text-white px-8 py-4 rounded-2xl font-black text-[10px] uppercase shadow-xl flex items-center gap-2 shrink-0 hover:bg-emerald-700 transition-colors shadow-emerald-100"><FileSpreadsheet className="w-5 h-5" /> Exportar Database</button>
              </div>
            </div>
          ...
          <div className="max-w-3xl mx-auto py-12 space-y-8">
            <div className="bg-white p-10 rounded-[50px] border border-slate-200 shadow-2xl space-y-8">
              ...
              <div className="space-y-6">
                ...
                <button onClick={() => saveGlobalSettings(globalConfig.ic_email, globalConfig.reference_cnpjs)} className="w-full bg-blue-600 text-white py-6 rounded-2xl font-black uppercase text-xs tracking-widest shadow-xl hover:bg-blue-700 transition-colors shadow-blue-200">Gravar Configurações Master</button>
              </div>
            </div>
          </div>
        )}
          ...
          <div className="max-w-md mx-auto py-12 animate-in slide-in-from-bottom-10">
            <div className="bg-white p-10 rounded-[50px] border border-slate-200 shadow-2xl space-y-8">
              ...
              <form onSubmit={e => { e.preventDefault(); handleCreateList(new FormData(e.currentTarget)); }} className="space-y-5">
                ...
                <button type="submit" disabled={isCreatingList} className="w-full bg-blue-600 text-white py-6 rounded-2xl font-black uppercase text-xs tracking-widest shadow-xl active:scale-90 hover:bg-blue-700 transition-all mt-4 disabled:opacity-50 shadow-blue-200">
      ...
      <footer className="fixed bottom-0 inset-x-0 bg-white border-t border-slate-100 p-6 flex justify-around items-center z-[100] shadow-lg">
          ...
          <button onClick={() => { if(currentListId) setActiveView('scanner'); else setActiveView('create-list'); }} className="bg-blue-600 text-white w-16 h-16 rounded-[25px] flex items-center justify-center -mt-14 border-8 border-slate-50 shadow-2xl transition-transform active:scale-90 shadow-blue-400/50"><Plus className="w-10 h-10" /></button>
          ...
          <button onClick={() => { if(currentUser?.role === 'admin') setActiveView('admin-settings'); else addNotification("Aviso", "Acesso restrito ao Administrador Master.", "info"); }} className={`flex flex-col items-center gap-1 transition-all ${activeView === 'admin-settings' ? 'text-blue-600 scale-110' : 'text-slate-300'}`}><Settings className="w-7 h-7"/><span className="text-[8px] font-black uppercase tracking-widest">Gestão</span></button>
      </footer>

      {isProcessing && (<div className="fixed inset-0 bg-slate-900/95 backdrop-blur-xl z-[300] flex flex-col items-center justify-center text-white text-center p-6 animate-in fade-in duration-500"><Loader2 className="w-20 h-20 text-blue-600 animate-spin mb-8" /><h3 className="text-2xl font-black uppercase italic tracking-tighter">Processando Inteligência PackScan</h3><p className="text-slate-400 text-[10px] font-bold uppercase mt-4 tracking-[0.2em] animate-pulse">Sincronizando com Base Master Global via IA</p></div>)}
      {zoomImage && (<div onClick={() => setZoomImage(null)} className="fixed inset-0 bg-black/95 z-[500] flex items-center justify-center p-4 cursor-zoom-out animate-in zoom-in-95"><img src={zoomImage} className="max-w-full max-h-full rounded-2xl shadow-2xl border-4 border-white/10" alt="Zoom" /></div>)}
    </div>
  );
};
